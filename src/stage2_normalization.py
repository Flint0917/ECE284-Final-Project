...
# This is a paired/intra-patient normalization, not a deployable standalone
# LOSO transform: the held-out patient's baseline is computed from the held-out
# patient's lesional and non-lesional traces together. It is useful for testing
# Todorov et al.'s intra-individual tracking claim, but its score should be
...
patient_mean_summary = []
for pid, group in df.groupby("patient"):
    les_mean = group.loc[group["label"] == 1, "trace"].iloc[0].mean()
    non_mean = group.loc[group["label"] == 0, "trace"].iloc[0].mean()
    patient_mean_summary.append((pid, les_mean, non_mean, les_mean - non_mean))
...
# Patient-level normalization can look perfect when every patient's lesional
# mean is above that patient's non-lesional mean after paired centering.
print("\nPatient-level normalization caveat:")
print("  Uses each held-out patient's own paired lesional + non-lesional traces")
print("  to define that patient's baseline. Treat as intra-patient/paired")
print("  normalization, not ordinary standalone cross-patient deployment.")
all_les_higher = all(diff > 0 for _, _, _, diff in patient_mean_summary)
print(f"  Lesional mean > non-lesional mean for all patients: {all_les_higher}")
